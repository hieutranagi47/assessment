...
## Implementation plan:
I want to get a list of available service_bay at a period time of a dealership.
After checking the DB design, it's quite complex to get the available service bay list. If you think that we need another table to track service bays, let discuss with me about that. if we add one more table, we will update the logic when the bay will be actually release (the appointment operation is done or the appointment is cancelled)
