...
func (s *availableServiceBaysRepositoryStub) IsActiveServiceTypeForAvailableServiceBays(context.Context, uuid.UUID, uuid.UUID) (bool, error) {
	return s.serviceTypeActive, nil
}

func (s *availableServiceBaysRepositoryStub) ListAvailableServiceBays(_ context.Context, dealershipID, serviceTypeID uuid.UUID, startsAt, endsAt time.Time) ([]domain.ServiceBay, error) {
...
func TestAvailableServiceBaysQueryListsBaysForHalfOpenInterval(t *testing.T) {
	...
	require.Equal(t, serviceTypeID, repository.serviceTypeID)
...
func TestAvailableServiceBaysQueryValidatesTimeRangeAndAccess(t *testing.T) {
	...
	repository := &availableServiceBaysRepositoryStub{allowed: false, active: true, serviceTypeActive: true}
	query := NewAvailableServiceBaysQuery(repository)
...
func TestAvailableServiceBaysQueryRequiresAnActiveServiceType(t *testing.T) {
	startsAt := time.Date(2026, 8, 29, 8, 0, 0, 0, time.UTC)
	repository := &availableServiceBaysRepositoryStub{
		allowed:           true,
		active:            true,
		serviceTypeActive: false,
	}
	query := NewAvailableServiceBaysQuery(repository)
	...
	var structured common.Error
	require.ErrorAs(t, err, &structured)
	require.Equal(t, "service_type_not_found", structured.ErrorSlug)
	require.False(t, repository.listWasCalled)
}
