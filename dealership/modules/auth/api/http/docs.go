...
// OpenAPI contract. The UI is intentionally part of the auth HTTP adapter so
// the module owns both its API routes and its documentation routes.
func RegisterDocs(router common.EchoRouter) {
	router.GET("/auth/docs", func(c *echo.Context) error {
		return c.Redirect(http.StatusTemporaryRedirect, "/auth/docs/")
	})
	router.GET("/auth/docs/", func(c *echo.Context) error {
		return c.HTML(http.StatusOK, swaggerUIHTML)
	})
	router.GET("/auth/docs/openapi.yaml", func(c *echo.Context) error {
