...
- Follow conventions of the project
...
Task:
Give me a prompt to create a staff for a dealership api:
Specs:
Only superadmin/admin (the user info is provided by GetUserInfo in /Users/hieutran/Development/Assessment/dealership/modules/auth/api/module/module.go) can create an admin user for the appointment_scheduler.users.
Request body: auth_user_id is the id in the auth.users table (user_id), dealership_id, ...
Then grant the user as the admin of the dealership
